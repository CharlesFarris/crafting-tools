    ...
    public static void ToResultNotEmpty_ValidatesBehavior()
    {
        // use case: empty guid
        {
            var result = Guid.Empty
                .ToResultNotEmpty(resultId: "empty");
            ...
            Assert.That(result.Id, Is.EqualTo(expected: "empty"));
        }
        ...
        // use case: non-empty guid
        {
            ...
            var result = id
                .ToResultNotEmpty(resultId: "non-empty");
            Assert.That(result, Is.Not.Null);
            Assert.That(result.Status, Is.EqualTo(ResultStatus.Success));
            Assert.That(result.Unwrap(), Is.EqualTo(id));
            Assert.That(result.Id, Is.EqualTo(expected: "non-empty"));
        }
    }
}
